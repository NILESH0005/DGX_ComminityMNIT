...
const hasUploadedFiles = (module) => {
  ...
  return module.subModules.some((subModule) =>
    subModule.units?.some((unit) => unit.files && unit.files.length > 0),
  );
};
...
const LearningMaterialManager = () => {
  ...
  const transformForBackend = async (moduleData) => {
    ...
    if (!moduleData.BatchID || isNaN(moduleData.BatchID)) {
      console.error("❌ BatchID is invalid:", moduleData.BatchID);
    ...
    return {
      ...
      LMSLevel: moduleData.LMSLevel || null,
      LMSUserCategory: moduleData.LMSUserCategory || null,
      ...
      if (uploadResponse.success) {
        const updatedModule = {
          ...
          subModules: formState.module.subModules.map((subModule) => {
            if (
              formState.subModule &&
              subModule.id === formState.subModule.id
            ) {
              return {
                ...subModule,
                units: subModule.units.map((unit) => {
  ...
  const handleSubmitAllData = async () => {
    ...
    try {
      ...
      if (response.success) {
        Swal.fire(
          "Success",
          "All learning materials submitted successfully",
          "success",
        );
      ...
      Swal.fire(
        "Error",
        error.message || "Failed to submit learning materials",
        "error",
      );
    } finally {
      setIsSubmitting(false);
  ...
  const handleModuleCreated = (newModule) => {
    ...
    const currentModules =
      JSON.parse(localStorage.getItem("learningMaterials")) || {};
  ...
  const handleUnitsUpdated = (subModuleId, units) => {
    ...
    saveToLocalStorage({
      ...formState.module,
      subModules: formState.module.subModules.map((subModule) =>
        subModule.id === subModuleId ? { ...subModule, units } : subModule,
      ),
    });
  };
        ...
        {formState.isCreatingModule ? (
          ...
          <UnitComponent
            ...
            onUnitsUpdated={(units) =>
              handleUnitsUpdated(formState.subModule.id, units)
            }
            onBack={() =>
              dispatch({ type: "SET_MODULE", payload: formState.module })
            }
